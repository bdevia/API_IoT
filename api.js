...
app.put('/api/v1/company/update', (req, res) => { //UPDATE
    const query = `SELECT * FROM Company WHERE company_name = '${req.body.company_name}';`;
    db.all(query, (error, rows) => {
        if (error) {
        console.error(error);
...
app.put('/api/v1/locations/update', (req, res) => { //UPDATE
    const query = `SELECT * FROM Company WHERE company_api_key ='${req.body.company_api_key}';`;
    db.all(query, (error, rows) => {
        if (error) {
        console.error(error);
            ...
            if(rows.length >= 1 ){
                ...
                db.all(query1, (error, rows) => {
                    if (error) {
                        ...
                        res.status(400).json({status: "failed", description: ` Location con id: ${req.body.location_id} no esta asociada a ninguna company`});
                    }
            ...
            if(rows.length >= 1 ){
                ...
                db.all(query1, (error, rows) => {
                   if(error) {
                    ...
                    if(rows.length >=1) {
                        const query2 = `DELETE FROM Sensor WHERE location_id = ${req.body.location_id} AND ID = ${req.body.sensor_id};`;
                        db.all(query2, (error, rows) => {
                    if (error) {
                        console.error(error);
                    } 
                        ...
                        if(rows.length >= 1){ //Se cumplen las 2 condiciones
                            ...
                            if (error) {
                                ...
                                if (rows.length >= 1 ){
                                    ...
                                    const query3 = `UPDATE Sensor SET sensor_name = '${req.body.sensor_name}', sensor_category = '${req.body.sensor_category}', sensor_meta = '${req.body.sensor_meta}' WHERE ID = '${req.body.sensor_id}';`; 
                                    db.run(query3, (error) => {
                                        if (error) {
                                            console.error(error);
                                        } 
...
app.post('/api/v1/sensor_data/insert', (req, res) => {
    ...
    db.all(query, (error, rows) => {
        if (error) {
            ...
            if (rows.length >= 1) {
                ...
                const insertDataQuery = `INSERT INTO Sensor_data (sensor_id, variable, valor) VALUES (?, ?, ?);`;
  ...
  app.get('/api/v1/sensor_data/read_all', (req, res) => { //READ_ALL
    const query = `SELECT * FROM Sensor WHERE sensor_api_key ='${req.body.sensor_api_key}';`;
    db.all(query, (error, rows) => {
        if (error) {
        console.error(error);
            ...
            if(rows.length >= 1 ){
                const sensorID = rows[0].ID;
                const query1 = `SELECT * FROM Sensor_data WHERE sensor_id = ${sensorID};`;
                db.all(query1, (error, rows) => {
                    if (error) {
                        console.error(error);
                    } 
                            ...
                            if(rows.length >= 1){
                                ...
                                db.all(query2, (error, rows) => {
                                    if (error) {
                                        ...
                                        if (rows.length >= 1 ){
                                            console.log(`Error, ya existe un Sensor_data con estas caracteristicas`);
                                            res.status(400).json({status: "failed", description: `ya existe un Sensor_data con estas caracteristicas`});
                                        }
                                        else{
                ...
                db.all(query1, (error, rows) => {
                   if(error) {
                    ...
                    if (error) {
                        ...
                        res.status(400).json({status: "failed", description: ` Sensor_data con id: ${req.body.sensor_data_id} no existe o no esta asociado a ningun Sensor`});
                    }
                   }
                })
